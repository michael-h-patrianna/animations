  ...
  return (
    <div className="coin-trail-container-css" data-animation-id="progress-bars__coin-trail">
      ...
      <div className="coin-track-css">
        <div className="coin-fill-css" style={{ width: `${progress}%` }} />

        {coins.map((pos) => {
          const isCollected = progress >= pos
          return (
            <div key={pos} className="coin-item-wrapper-css" style={{ left: `${pos}%` }}>
              <div className={`coin-item-css ${isCollected ? 'collected' : ''}`}>🟡</div>
              {isCollected && <div className="coin-plus-one-css">+10</div>}
            </div>
          )
        })}
      </div>
