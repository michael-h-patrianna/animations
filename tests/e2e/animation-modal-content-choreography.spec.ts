...
test.describe('Modal Content', () => {
  test('list-soft-stagger renders a modal with staggered list items', async ({ catalogPage }) => {
    await catalogPage.gotoGroup('modal-content-choreography-framer')

    const card = catalogPage.card('modal-content-choreography__list-soft-stagger')
  ...
  test('content stagger components have meaningful dimensions', async ({ catalogPage }) => {
    ...
    const card = catalogPage.card('modal-content-choreography__buttons-stagger-2')
    const stage = await catalogPage.cardStage(card)
  ...
  test('all framer variants render with visible content', async ({ catalogPage }) => {
    ...
    const cards = catalogPage.scopedCards('modal-content-choreography-framer')
  ...
  test('CSS and Framer variants produce matching animation IDs', async ({ catalogPage }) => {
    ...
    await catalogPage.gotoGroup('modal-content-choreography-css')
