...
test.describe('Epic Win Animation', () => {
  test('framer variant renders layered text structure with chars and glows', async ({
    ...
    const container = stage.locator('.pf-epic-win')
    await expect(container).toBeVisible()

    // 8 chars in "EPIC WIN" (including space)
    await expect(container.locator('.pf-epic-win__char')).toHaveCount(8)
    await expect(container.locator('.pf-epic-win__char-glow')).toHaveCount(8)

    // Shadow layers exist for depth effect
    await expect(container.locator('.pf-epic-win__shadow-far')).toBeVisible()
    await expect(container.locator('.pf-epic-win__shadow-mid')).toBeVisible()
  })
  ...
  test('replay remounts epic-win in both variants', async ({ catalogPage }) => {
    const variants = [
      { groupId: 'text-effects-framer', rootSelector: '.pf-epic-win' },
