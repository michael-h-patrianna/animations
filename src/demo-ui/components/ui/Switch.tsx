...
function SwitchThumb({ checked, iconOn, iconOff }: Pick<SwitchProps, 'checked' | 'iconOn' | 'iconOff'>) {
  return (
    <m.div
      initial={false}
    ...
    >
      <div className='relative w-full h-full flex items-center justify-center'>
        {Boolean(iconOn) && (
          <m.div initial={false} animate={{ opacity: checked ? 1 : 0, scale: checked ? 1 : 0.5 }} className='absolute text-accent text-[10px]'>
            {iconOn}
          </m.div>
        )}
        {Boolean(iconOff) && (
          <m.div initial={false} animate={{ opacity: checked ? 0 : 1, scale: checked ? 0.5 : 1 }} className='absolute text-background text-[10px]'>
            {iconOff}
          </m.div>
        )}
      </div>
...
export const Switch: React.FC<SwitchProps> = ({
  ...
  return (
    <label
      ...
      onMouseEnter={() => { if (!disabled) soundManager.playHover() }}
      data-testid={dataTestId}
    >
      <div className='relative isolate w-11 h-6'>
        <m.input
          ...
          className='sr-only'
          checked={checked}
          onChange={(e) => { if (!disabled) { onCheckedChange(e.target.checked); soundManager.playClick() } }}
          disabled={disabled}
          role='switch'
          aria-checked={checked}
        />
        <div className={`absolute inset-0 rounded-full border transition-all duration-300 ease-out ${trackClass}`} />
        <SwitchThumb checked={checked} iconOn={iconOn} iconOff={iconOff} />
      </div>
      {label && <span className='text-xs font-medium text-text-secondary group-hover/switch:text-text-primary transition-colors tracking-wide'>{label}</span>}
    </label>
  )
}
