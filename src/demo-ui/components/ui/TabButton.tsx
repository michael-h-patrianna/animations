...
export const TabButton: React.FC<TabButtonProps> = ({
  tab, isActive, instanceId, variant, fullWidth, testId, onSelect, onKeyDown, index, buttonRef,
}) => (
  <m.button
    ref={buttonRef}
    type='button' role='tab' id={`tab-${tab.id}`} aria-selected={isActive} aria-controls={`panel-${tab.id}`} tabIndex={isActive ? 0 : -1}
    onClick={() => { if (!isActive) soundManager.playClick(); onSelect(tab.id) }}
    onMouseEnter={() => { if (!isActive) soundManager.playHover() }}
  ...
  >
    {isActive && variant !== 'pills' && <m.div layoutId={`activeTab-${instanceId}`} className='absolute bottom-[-1px] inset-inline-0 h-[2px] bg-accent shadow-[0_0_8px_var(--color-accent)]' transition={{ type: 'spring', bounce: 0.2, duration: 0.5 }} />}
    {isActive && variant !== 'pills' && <div className='absolute inset-0 bg-gradient-to-t from-accent/5 to-transparent pointer-events-none' />}
