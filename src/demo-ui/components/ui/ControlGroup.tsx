...
function GroupHeader({
  ...
  return (
    <div
      ...
      onClick={onToggle}
      onMouseEnter={() => { if (collapsible) soundManager.playHover() }}
      onKeyDown={(e) => { if (collapsible && (e.key === 'Enter' || e.key === ' ')) { e.preventDefault(); onToggle() } }}
    >
      <div className='flex items-center gap-2'>
        {collapsible && (
          <m.div animate={{ rotate: isOpen ? 90 : 0 }} transition={{ duration: 0.2 }} className='text-text-tertiary'>
            <svg width='10' height='10' viewBox='0 0 24 24' fill='none' stroke='currentColor' strokeWidth='2' strokeLinecap='round' strokeLinejoin='round'>
              <path d='M9 18l6-6-6-6' />
            </svg>
          </m.div>
        )}
        <span className={`text-xs font-semibold uppercase tracking-wider ${collapsible ? 'text-text-secondary' : 'text-text-secondary'}`}>
          {title}
        </span>
      </div>
...
export const ControlGroup: React.FC<ControlGroupProps> = ({
  ...
  return (
    <div className={`border-b border-[var(--border-subtle)] pb-2 last:border-0 ${className}`} data-testid={testId}>
      {showTitle && (
        <GroupHeader title={title} collapsible={collapsible} isOpen={isOpen} rightElement={rightElement} onToggle={toggle} />
      )}
