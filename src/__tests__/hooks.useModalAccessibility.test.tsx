import { useFocusTrap, useEscapeClose, useOverlayDismiss } from '@/hooks/useModalAccessibility'
...
describe('useOverlayDismiss', () => {
  it('calls onClose when clicking directly on the overlay element', () => {
    const onClose = vi.fn()

    function TestComponent() {
      const overlayRef = useRef<HTMLDivElement>(null)
      const handleClick = useOverlayDismiss(overlayRef, onClose)
  ...
  it('does NOT call onClose when clicking on overlay children (modal content)', () => {
    const onClose = vi.fn()
    ...
    // Click content (child of overlay) — should NOT close
    fireEvent.click(screen.getByTestId('content'))
    expect(onClose).not.toHaveBeenCalled()
  })

  it('returns a stable click handler across re-renders when refs are stable', () => {
    const onClose = vi.fn()

    function TestComponent() {
      const overlayRef = useRef<HTMLDivElement>(null)
      const handleClick = useOverlayDismiss(overlayRef, onClose)
      return <div ref={overlayRef} onClick={handleClick} />
    }

    const { rerender } = render(<TestComponent />)
    // Re-render with same props — handler should be memoized
    rerender(<TestComponent />)
    // No assertion on reference stability since useCallback with ref deps is used,
    // but verify it still works
    expect(onClose).not.toHaveBeenCalled()
  })

  it('handles null overlayRef gracefully (no crash on click)', () => {
    const onClose = vi.fn()
    ...
    function TestComponent() {
      ...
      const handleClick = useOverlayDismiss(overlayRef, onClose)
      return (
        <div data-testid="unrefed" onClick={handleClick}>
          Content
        </div>
      )
    }
    ...
    // Click the div — overlayRef.current is null, so e.target !== null → no close
    fireEvent.click(screen.getByTestId('unrefed'))
    expect(onClose).not.toHaveBeenCalled()
  })
})
