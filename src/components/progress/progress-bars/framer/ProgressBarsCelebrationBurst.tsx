...
export function ProgressBarsCelebrationBurst() {
  ...
  const triggerMilestone = useCallback(
    (index: number) => {
      ...
      setMilestoneAnimations((prev) => [...prev, { id: milestoneId, index }]) // Create particles for this milestone
      const newParticles: Particle[] = [0, 90, 180, 270].map((angle) => ({
        id: particleIdRef.current++,
        milestoneIndex: index,
        angle,
      }))
      setParticles((prev) => [...prev, ...newParticles]) // Remove particles after animation completes
      registerTimeout(() => {
        setParticles((prev) => prev.filter((p) => !newParticles.find((np) => np.id === p.id)))
      }, 500) // Remove milestone animation marker
      registerTimeout(() => {
  ...
  useEffect(() => {
    ...
    const startAnimation = () => {
      if (stopped) return
      resetAnimation() // Animate from 0 to 100% over 4 seconds
  ...
  const burstRingVariants = (delay: number) => ({
    ...
    active: {
      scale: 2,
      opacity: [0.6, 0],
      transition: { duration: 0.3, delay, ease: [0.2, 0.8, 0.2, 1] as const },
    },
  })
  const particleVariants = (angle: number) => {
    ...
    return {
      ...
      animate: {
        ...
        x,
        y,
        transition: { duration: 0.4, times: [0, 0.3, 1], ease: 'easeOut' as const },
      },
    }
  }
  return (
