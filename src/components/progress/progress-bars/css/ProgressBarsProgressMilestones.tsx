...
export function ProgressBarsProgressMilestones() {
  ...
  useEffect(() => {
    ...
    fill.style.background = 'linear-gradient(90deg, #c6ff77 0%, #d4ff9f 100%)' // Create milestone positions
      ...
      milestoneMarkers.forEach(({ marker, innerGlow, ring, position, label }, index) => {
        if (progress >= position && !activatedMilestones.has(index)) {
          ...
          marker.style.boxShadow =
            '0 0 25px var(--pf-anim-cyan-light), 0 0 50px rgba(var(--pf-anim-green-rgb, 198,255,119),0.5)' // Glow effect
