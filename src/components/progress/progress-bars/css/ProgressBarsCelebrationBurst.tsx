...
export function ProgressBarsCelebrationBurst() {
  ...
  const [_milestoneAnimations, setMilestoneAnimations] = useState<MilestoneAnimation[]>([])
  ...
  const triggerMilestone = useCallback(
    (index: number, markerContainer: HTMLElement, marker: HTMLElement) => {
      ...
      setMilestoneAnimations((prev) => [...prev, { id: milestoneId, index }]) // Activate marker with scale animation
      marker.style.transform = 'scale(1.3)'
      marker.style.backgroundColor = 'var(--pf-anim-purple)'
      marker.style.opacity = '1'
      registerTimeout(() => {
        marker.style.transform = 'scale(1)'
      }, 150) // Create burst rings
      for (let i = 0; i < 2; i++) {
  ...
  useEffect(() => {
    ...
    const resetAnimation = () => {
      ...
      lastProgressRef.current = 0 // Reset fill
    ...
    const startAnimation = () => {
      if (stopped) return
      resetAnimation() // Create milestone markers
      ...
      milestonePositions.forEach((pos, index) => {
        ...
        trackContainer.appendChild(markerContainer) // Main marker
        ...
        milestoneMarkers.push({ container: markerContainer, marker, position: pos, index })
