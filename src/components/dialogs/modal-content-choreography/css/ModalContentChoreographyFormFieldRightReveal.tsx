 ...
 *
 * Copy-paste files: this file + ModalContentChoreographyFormFieldRightReveal.module.css + ../shared.css + ../SharedTypes.ts
 * Runtime deps: react
 */
...
import styles from './ModalContentChoreographyFormFieldRightReveal.module.css'
...
function ModalContentChoreographyFormFieldRightRevealComponent({
  children,
  ...
  const wrapItem = (child: React.ReactNode, i: number, delayBase: number) => (
    <div
      key={i}
      className={styles['pf-form-right-reveal-item']}
      style={
        {
  ...
  if (items.length > 0) {
    return (
      <div
        ...
        data-animation-id="modal-content-choreography__form-field-right-reveal"
        style={style}
      >
...
export const ModalContentChoreographyFormFieldRightReveal = memo(
  ModalContentChoreographyFormFieldRightRevealComponent
)
