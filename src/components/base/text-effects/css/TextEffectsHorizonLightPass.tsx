...
function TextEffectsHorizonLightPassComponent({
  ...
  return (
    <div
      ref={containerRef}
      className={styles['pf-tfx-horizon-light-pass']}
    ...
    >
      <div className={styles['pf-tfx-horizon-light-pass__line']} aria-hidden="true">
        {letters.map((ch, i) => (
          <span
            key={i}
            ref={(el) => {
            ...
            className={styles['pf-tfx-horizon-light-pass__letter']}
          >
            {ch === ' ' ? '\u00A0' : ch}
          </span>
        ))}
