...
interface TextEffectsCounterIncrementProps {
  /** Target value to count to. If not set, runs in continuous demo mode. @default undefined */ targetValue?: number
  /** Total animation duration in milliseconds. @default 2000 for continuous, 3000 for target mode */ durationMs?: number
  /** Number of increment steps/particles to show. @default 1 for continuous, 8 for target mode */ maxParticles?: number
  /** Animation ID for data-animation-id attribute. @default 'text-effects__counter-increment' */ animationId?: string
  /** Legacy: Fixed increment value for continuous mode. @deprecated Use durationMs and maxParticles instead */ incrementValue?: number
  /** Legacy: Time between increments. @deprecated Use durationMs instead */ intervalMs?: number
}
 ...
 */ function roundToNiceNumber(num: number): number {
  if (num === 0) return 0 // Find magnitude using string length (faster than log10)
  const numStr = Math.abs(num).toString()
  const magnitude = 10 ** (numStr.length - 1)
  const normalized = num / magnitude // Round to 1, 2, 2.5, or 5 * magnitude
 ...
 */ function calculateIncrementSteps(
  ...
  let cumulativeValue = 0 // Ease-in cubic curve: y = x^3
  ...
  for (let i = 0; i < numSteps; i++) {
    ...
    const easedProgress = easeInCubic(progress) // Calculate target value at this step (cumulative)
    ...
    cumulativeValue += incrementAmount // Calculate cumulative timing using ease-in
  ...
  if (cumulativeValue < targetValue) {
    ...
    if (steps.length > 0) {
      ...
      steps.push({ value: targetValue, timing: durationMs, incrementAmount: targetValue })
    }
  }
  return steps
}
 ...
 */ function TextEffectsCounterIncrementComponent({
  ...
  durationMs,
  maxParticles,
  animationId = 'text-effects__counter-increment', // Legacy props for backward compatibility
  incrementValue,
  intervalMs,
  ...
  const popAnimationKeyRef = useRef(0) // Determine if running in continuous mode or target mode
  ...
  const effectiveMaxParticles = maxParticles ?? (isContinuousMode ? 1 : 8) // For continuous mode, use simple increment
  ...
  useEffect(() => {
    ...
    steps.forEach((step: IncrementStep) => {
      const timeoutId = setTimeout(() => {
        setCount(step.value)
        popAnimationKeyRef.current++ // Add particle
        const particleId = nextParticleIdRef.current++
        const newParticle = { id: particleId, value: step.incrementAmount }
        setParticles((prev) => {
          const newParticles = [...prev, newParticle] // Keep only recent particles (they auto-remove via animation)
